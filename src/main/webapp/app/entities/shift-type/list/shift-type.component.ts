...
export class ShiftTypeComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.shiftTypeService
      ...
      .subscribe({
        next: (res: HttpResponse<IShiftType[]>) => {
  ...
  trackId(_index: number, item: IShiftType): number {
    return item.id!;
  }

  delete(shiftType: IShiftType): void {
    const modalRef = this.modalService.open(ShiftTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.shiftType = shiftType;
  ...
  protected onSuccess(data: IShiftType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
